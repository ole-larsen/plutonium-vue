...
// @ts-ignore
const info: any = computed(() => store.info);
const menu : any = computed(() => store.menu);
        ...
        <div class="col-lg-3 col-md-12 col-12">
          <div class="widget widget-logo">
            <div class="logo-footer" id="logo-footer">
              <router-link to="/" v-if="info">
                <img :src="logo" :alt="info.alt" id="logo-footer-img">
              </router-link>
            </div>
            <p class="sub-widget-logo">Digital marketplace for crypto collectibles and non-fungible tokens (NFTs).</p>
            <p class="sub-widget-logo">Buy, sell, and discover exclusive digital items.</p>
          </div>
        </div>
        <template v-if="menu && menu.items" v-for="(item, itemIndex) in menu.items">
          <div
            :class="itemIndex === 0
              ? `col-lg-2 col-md-4 col-sm-5 col-5`
              : (itemIndex === 1 ? `col-lg-2 col-md-4 col-sm-7 col-7` : `col-lg-2 col-md-4 col-sm-5 col-5`)" >
            <div class="widget widget-menu" :class="`style-${itemIndex + 1}`" v-if="item.attributes">
              ...
              <ul class="quick-links" v-if="item.attributes.items">
                <li v-for="link in item.attributes.items" :key="link.id">
                  <router-link :to="link.attributes.link">{{link.attributes.name}}</router-link>
                </li>
              </ul>
            </div>
          </div>
        ...
        <div class="col-lg-3 col-md-6 col-sm-7 col-12">
          <div class="widget widget-subcribe">
            ...
            <div class="form-subcribe">
              <form id="subscribe-form" action="#" accept-charset="utf-8" class="form-submit">
                <input type="hidden" name="csrf" :value="form.csrf"/>
                <input name="email"
                       v-model="form.email"
                       class="email" type="email" placeholder="info@your-mail.com" required />
                <button id="submit"
                        :disabled="form.email === ``"
                        name="submit" type="submit"
                        @click.prevent="submit"><i class="icon-fl-send"></i></button>
              </form>
            </div>
      ...
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body space-y-20 pd-40" :class="{ show: isActive }">
            <h3>Place a Bid</h3>
            <p class="text-center">You must bid at least <span class="price color-popup">4.89 ETH</span></p>
