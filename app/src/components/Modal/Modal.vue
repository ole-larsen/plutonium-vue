...
watch(() => price.value, (_price: string) => {
  if (collectible && _price && isFinite(Number(_price)) && !isNaN(Number(_price))) {
    ...
    collectible.value.attributes.details.price_wei = ethers.utils.parseEther(collectible.value.attributes.details.price);
    collectible.value.attributes.details.total_wei = ethers.utils.parseEther(collectible.value.attributes.details.total);
  }
});
...
<template>
  <div v-if="collectible && collectible['attributes']" class="modal fade popup" id="popup_bid" tabIndex="-1" role="dialog" aria-hidden="true" :class="{ show: isActive }">
    <div class="overlay" v-on:click="close"></div>
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close" v-on:click="close"></button>
        <div class="modal-body space-y-20 pd-40" :class="{ show: isActive }">
            ...
            <input type="text" class="form-control"
              :placeholder="collectible['attributes']['details']['total'] + ' ETH'"
              v-model="price"/>
            ...
            <div class="d-flex justify-content-between">
              ...
              <p class="text-right price color-popup"> {{ collectible['attributes']['details']['price'] }} ETH </p>
            </div>

            <div class="d-flex justify-content-between">
              <p>Service free:</p>
              <p class="text-right price color-popup"> {{ collectible['attributes']['details']['fee'] }} % </p>
            </div>
            <div class="d-flex justify-content-between">
              <p>You must bid at least:</p>
              <p class="text-right price color-popup"> {{ (+collectible['attributes']['details']['total']).toFixed(4) }} ETH </p>
            </div>
            <button class="sc-button style-place-bid style bag fl-button pri-3" @click.prevent="placeBid"><span>Place a bid</span></button>
