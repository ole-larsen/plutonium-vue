...
//   item.value = todayPicks.value.find(
//     (_item: any) => _item.id === itemId
//   ) as any;
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="heading-live-auctions mg-bt-21">
            ...
            &lt;!&ndash;<router-link
              :to="todayPicks.btnLink"
              class="exp style2"
              >{{ todayPicks.btnText }}</router-link
            >&ndash;&gt;
          </div>
        </div>
        ...
        <div
          ...
          v-for="item in todayPicks"
          :key="item['id']"
        >
          <div
            v-if="item"
            class="sc-card-product"
            :class="`${item['coomingsoon'] ? 'comingsoon' : ''}`"
          >
            <div class="card-media">
              <router-link :to="`/card/${item['id']}`" v-if="item['metadata']">
                <img
                  :src="item['metadata']['image']"
                  :alt="item['metadata']['name']"
              /></router-link>
              &lt;!&ndash;
              <router-link to="/login" class="wishlist-button heart"
                ><span class="number-like">100</span></router-link
              >&ndash;&gt; &lt;!&ndash;
