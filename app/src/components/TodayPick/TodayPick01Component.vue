...
// import Modal from "@/components/template/Modal/ModalComponent.vue";
...
// const items: ComputedRef<PublicMarketItem[]> = computed(() =>
//   market.items.filter((_item: PublicMarketItem) => !_item.metadata.auction)
// );
      ...
      <div class="row">
        ...
        >
          <div
            ...
            :class="`${_item['comingsoon'] ? 'comingsoon' : ''}`"
          >
            <div class="card-media">
              <router-link
                :to="_item['metadata']['url']"
                v-if="_item['metadata']"
              >
                <img
                  :src="_item['metadata']['image']"
                  :alt="_item['metadata']['name']"
              /></router-link>
            ...
            <div class="card-title">
              <h5 class="style2">
                <router-link :to="_item['metadata']['url']"
                  >"{{ _item["metadata"]["name"] }}"</router-link
                >
              </h5>
              <div class="tags">
                {{
                  _item["metadata"]["fulfilled"]
                    ? "sold"
                    : _item["metadata"]["tags"]
                }}
              </div>
            ...
            <div
              class="meta-info"
              v-if="
                _item && _item['owner'] && typeof _item['owner'] !== 'string'
              "
            >
              <div class="author">
                <div class="avatar">
            ...
            <div
              class="card-bottom"
              :class="`${_item['comingsoon'] ? 'none' : ''}`"
              v-if="!_item['metadata']['fulfilled']"
            >
              <button
                class="sc-button style bag fl-button pri-3 no-bg"
                v-on:click="toggleActive(_item)"
              >
                <span>Buy</span>
              </button>
              <router-link to="/activity-01" class="view-history reload"
                >History</router-link
              >
            </div>
          </div>
        ...
        <div
          class="col-md-12 wrap-inner load-more text-center"
          v-if="items.length > 10"
        >
          <router-link
            to="#"
            id="load-more"
            class="sc-button loadmore fl-button pri-3"
            ><span>Load More</span></router-link
          >
        </div>
      </div>
    </div>
