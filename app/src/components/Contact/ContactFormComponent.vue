...
<template>
  <div v-if="sent === false">
    <form id="contactform" class="form-submit contact-form">
      <h2
        class="tf-title-heading style-2 mg-bt-12"
        v-if="heading"
        v-html="heading"
      ></h2>
      ...
      <label
        class="contact-form_email error"
        v-if="errors.email"
        v-html="errors.email"
      ></label>
      <input
      ...
      <div class="row-form style-2">
        <label
          class="contact-form_subject error"
          v-if="errors.subject"
          v-html="errors.subject"
        ></label>
        <select id="subject" v-model="form.subject">
          <option
            v-for="item in form.subjectItems"
      ...
      <label
        class="contact-form_message error"
        v-if="errors.message"
      ...
      ></textarea>
      <button class="submit" @click.prevent="submit" :disabled="disabled">
        Send message
      </button>
    </form>
  </div>
