...
function mint() {
  error.value = "";
  if (collectible && collectible.value) {
...
if (collectible) {
  watch(
    ...
    (collectionId: number) => {
      const collection = collections?.value.find((_collection: PublicCategoryCollection) => _collection.id === collectionId);
      if (collection) {
        collectible.value.name = `${collection.attributes.name} #${collection.attributes.collectibles.length + 1}`
      }
    })
}
  ...
  <div class="mint-form" v-if="collectible">
    ...
    <div>
      <div class="row-form style-2">
        ...
        <select class="form-select" aria-label="Select Collection" v-model="collectible.collectionId">
          <option value="0">Select Collection</option>
          <option v-for="collection in collections" :value="collection.id">{{ collection.attributes.name }}</option>
        </select>
      </div>
      ...
      <div class="row-form style-2">
        ...
        <input id="tags" name="tags" tabIndex="2" aria-required="true" type="text" v-model="collectible.tags" placeholder="NFT Tags" />
      </div>
      <div class="row-form style-2">
        <label class="form-label" for="auction">Name</label>
        <input id="name" name="name" tabIndex="2" aria-required="true" type="text" v-model="collectible.name" placeholder="NFT Name" />
      </div>
      <div class="row-form style-2">
        <label class="form-label" for="description">Description</label>
        <textarea id="description" name="description" tabIndex="2"  aria-required="true" type="text"
                placeholder="NFT Description" v-model="collectible.description"/>
      </div>
      <div class="row-form style-2">
        ...
        <input id="price" name="price" tabIndex="2"  aria-required="true" type="number"
             placeholder="NFT Price" v-model="price"/>
      </div>
      <div class="row-form style-2">
        <label class="form-label" for="price">Quantity</label>
        <input id="quantity" name="quantity" tabIndex="2"  aria-required="true" type="number"
               placeholder="NFT quantity" v-model="collectible.quantity"/>
      </div>
      ...
      <input type="checkbox" name="auction" tabIndex="2"  aria-required="true"
             placeholder="Auction" ref="auction" v-model="collectible.auction" />


      <div v-if="collectible.auction">
        <div class="row-form style-2">
          <label class="form-label" for="start_price">Start Price</label>
          <input id="start_price" name="start_price" tabIndex="2"  aria-required="true" type="number"
               placeholder="Start Price" v-model="startPrice"/>
        </div>
        <div class="row-form style-2">
          <label class="form-label" for="reserve_price">Reserve Price</label>
          <input id="reserve_price" name="reserve_price" tabIndex="2"  aria-required="true" type="number"
               placeholder="reserve Price" v-model="reservePrice"/>
        </div>
        <div class="row-form style-2">
          <label class="form-label">Auction Start</label>
          <datepicker v-model="startTime" minimumView="time" inputFormat="yyyy-MM-dd HH:mm"/>
        </div>
        <div class="row-form style-2">
          <label class="form-label">Auction End</label>
          <datepicker v-model="endTime" minimumView="time" inputFormat="yyyy-MM-dd HH:mm"/>
        </div>
      </div>
      <br/>
