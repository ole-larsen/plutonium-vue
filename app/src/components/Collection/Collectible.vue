...
const auctionContract: ComputedRef<NFTAuction> = computed(() => {
  const auctionName = "auction_" + collectible?.value.attributes.collectionId + ":" + collectible?.value.attributes.itemId;
...
const startTimestamp = collectible?.value['attributes']['details']['start_time'];
...
<template>
  <div class="sc-card-product explode ">
    <div class="card-media">
      ...
      <div class="coming-soon">{{ collectible['attributes']['tokenIds'].length }}</div>
      ...
      <template v-if="collectible['attributes']['details']['auction']">
        ...
        <div class="button-place-bid" v-if="authorized && isOwner && !isStarted">
          <button class="sc-button style-place-bid style bag fl-button pri-3" @click="startAuction"><span>Start Auction</span></button>
        </div>
      </template>
...
<!--      </router-link>-->
    ...
    <div>
      <div class="meta-info" v-if="collectible['attributes']['creator']">
        <div class="author">
          ...
          <div class="info">
            <span>Creator</span>
            <h6>
              <router-link :to="`/author/${collectible['attributes']['creator']['username']}`">
                {{collectible['attributes']['creator']['username'].length > 10 ? collectible['attributes']['creator']['username'].slice(0, 4) + "..." + collectible['attributes']['creator']['username'].slice(-4) : collectible['attributes']['creator']['username']}}
              </router-link>
            </h6>
          </div>
        </div>
        <div class="price" v-if="collectible['attributes']['details']['auction']">
      ...
      <div class="meta-info" v-if="collectible['attributes']['owner']">
        <div class="author">
          <div class="avatar">
            <img :src="collectible['attributes']['owner']['gravatar']" :alt="collectible['attributes']['owner']['username']">
          </div>
          <div class="info">
            <span>Owner</span>
            <h6>
              <router-link :to="`/author/${collectible['attributes']['owner']['username']}`">
                {{collectible['attributes']['owner']['username'].length > 10 ? collectible['attributes']['owner']['username'].slice(0, 4) + "..." + collectible['attributes']['owner']['username'].slice(-4) : collectible['attributes']['owner']['username']}}
              </router-link>
            </h6>
          </div>
        </div>
      ...
      <div class="card-bottom" :class="`active`" v-if="authorized && isStarted && collectible['attributes']['details']['auction'] && startTimestamp && endTimestamp">
        <button class="sc-button style bag fl-button pri-3 no-bg"
                v-if="collectible['attributes']['details']['auction']" @click.prevent="placeBid">
          <span>Place Bid</span>
        </button>

        <router-link to="#" class="view-history reload" @click.prevent="loadHistory">Load history</router-link>
      </div>
      <div class="card-bottom" :class="`active`" v-if="authorized && !collectible['attributes']['details']['auction']">
        <button class="sc-button style bag fl-button pri-3 no-bg" @click.prevent="buyCollectible">
          <span>Buy</span>
        </button>
