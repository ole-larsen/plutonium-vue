...
const blogs: ComputedRef<{ [id: string]: BlogItem }> = computed(() =>
  store.getBlogs()
);

const recent: Ref<BlogItem[]> = ref([]);
const blog: ComputedRef<BlogItem> = computed(() =>
  store.getBlog(String(store.getPath()))
);
const hashtags = computed(
  () =>
    blog.value["tags"] ?? [].map((tag: BlogItemTag) => tag["title"]).join(",")
);
        ...
        <div class="post">
          <div class="inner-content">
                  ...
                  <div class="meta-info">
                    <div class="author">
                      <div
                        class="avatar"
                        v-if="
                          blog['author']['attributes']['image']['attributes'][
                            'url'
                          ]
                        "
                      >
            ...
            <div class="sc-widget style-1">
              <div
                class="widget widget-tag style-2"
                v-if="blog['tags'] && blog['tags'].length > 0"
              >
                <h4 class="title-widget">Tags</h4>
                <ul v-for="tag in blog['tags']" v-bind:key="tag['title']">
                  <li>
              ...
              <div class="widget widget-social style-2">
                ...
                <ul>
                  ...
                  <li>
                    <router-link to="#" class="icon-fl-facebook"></router-link>
                  </li>
                  <li class="style-2">
                    <router-link to="#" class="icon-fl-coolicon"></router-link>
                  </li>
                  <li class="mgr-none">
                    <router-link to="#" class="icon-fl-mess"></router-link>
                  </li>
                </ul>
              </div>
            </div>
        ...
        <div class="side-bar details">
            ...
            <ul v-if="recent">
              ...
              >
                <div class="box-feature">
                  <router-link :to="'/blog/' + _blog['link']">
                    <img
                      :src="_blog['image']['attributes']['url']"
                      alt="image"
                    />
                  </router-link>
                </div>
                <div class="box-content">
                  ...
                  <div>
                    <span
                      class="sub-recent-post"
                      v-html="_blog['description']"
                    ></span>
                    <router-link to="/blog" class="day-recent-post">{{
                      _blog["date"]
                    }}</router-link>
                  </div>
                </div>
          ...
          <div class="widget widget-tag style-1">
            ...
            <ul v-if="blog['popularTags']">
              <li v-for="tag in blog['popularTags']" v-bind:key="tag['link']">
                <router-link class="box-widget-tag" :to="tag['link']">{{
                  tag["title"]
                }}</router-link>
              </li>
            </ul>
