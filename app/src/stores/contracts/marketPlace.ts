...
export const useMarketPlaceStore = defineStore("marketPlace", () => {
  const instance: Ref<PublicContractDto | null> = ref(null);
  const contract: Ref<any> = ref(null);
  const categories: Ref<PublicCategoryDto[]> = ref([]);

  function initMarketplaceContract(publicContract: PublicContractDto): void {
    instance.value = publicContract;
  }

  function loadMarketplaceContract(signer: ethers.Signer) {
    if (instance.value) {
      contract.value = new ethers.Contract(
        instance.value?.address,
        JSON.parse(instance.value?.abi),
        signer
      ) as Marketplace; 
    }
  }

  function loadCategories(publicCategories: PublicCategoryDto[]): void {
    categories.value = publicCategories.map((publicCategory: PublicCategoryDto) => {
      if (publicCategory.attributes.collections) {
        publicCategory.attributes.collections = publicCategory.attributes.collections.map(
          (collection: MarketplaceCollectionDto | null) => {
            if (collection) {
              collection.attributes.category = {
                title: publicCategory.attributes.title,
                url: publicCategory.attributes.slug,
              };  
            }
            return collection;
          }
  ...
  function getCategories(): PublicCategoryDto[] {
  ...
  // const collections: Ref<MarketplaceCollectionDto[]> = ref([]);
  ...
  // function loadMarketplaceContractToMetamask() {
  //   contract.value = new ethers.Contract(
  ...
  // function loadMarketplaceContractToWeb3() {
  //   contract.value = new ethers.Contract(
  //     contractAddress.value,
  //     JSON.parse(abi.value),
  //     web3.signer()
  ...
  //   //   (total: MarketplaceCollectionDto[], next: PublicCategoryDto) =>
  ...
  // function getCollections(): MarketplaceCollectionDto[] {
  ...
  //             logo: (collection.logo as PublicFileDto),
  //             featured: (collection.featured as PublicFileDto),
  //             banner: (collection.banner as PublicFileDto),
  //           });
  ...
  // async function placeBid(collectible: PublicCategoryCollectionCollectibleDto) {
  //   const contract =
  //     auction.contract[
  //       "auction_" +
  ...
  //       console.log(user.value.attributes.address);
  //     }
  //     // console.log(await tx.wait());
  //     // console.log(await contract.highestBidder());
  //     // console.log(await contract.highestBid());
  ...
  return {
    ...
    loadCategories,
    getCategories,
    // loadCollections: storeCollections,
    // getCollections,
    // getName,
    // getOwner,
    // contractAddress,
    // startAuction,
    // loadMetamaskContract: loadMarketplaceContractToMetamask,
    // loadWeb3Contract: loadMarketplaceContractToWeb3,
    // mintCollection,
    // editCollection,
    // mintERC721,
    // placeBid,
